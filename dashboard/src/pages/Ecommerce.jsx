...
import { Stacked, Pie, Button, SparklineAreaData, ecomPieChartData } from '../components'
        ...
        rounded-xl w-full lg:w-80 p-8 pt-9 m-3 bg-hero-pattern bg-no-repeat bg-cover bg-center">
          ...
          <div className="mt-6">
            <Button color="white" bgColor="blue" text="Download"
              boarderRadius="10px" size="md" />
          </div>
        </div>
        <div className="flex m-3 flex-wrap justify-center gap-1 items-center">
          {earningData.map((item) => (
            <div key={item.title} className="bg-white dark:text-gray-50 
            dark:bg-secondary-dark-bg md:w-56 p-4 pt-9 rounded-2x1">
              <button type="button" style={{ color: item.iconColor, backgroundColor: item.iconBg }}
                className="text-2xl opacity-0.9 rounded-full hover:drop-shadow-x1">
                {item.icon}
              </button>
              <p className="mt-3">
